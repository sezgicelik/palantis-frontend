...
function renderAyarlar(el, data) {
  ...
  el.innerHTML =
    ...
    '<div class="card" id="ayar-sec-telegram">' +
      ...
      (tg.bagli ?
        '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">' +
          ...
          '</div>' +
          '<button class="btn-action" style="width:auto;padding:4px 12px;font-size:11px;background:#27ae60;color:#fff;margin-left:8px" onclick="telegramTest()">Test Gonder</button>' +
          '<button class="btn-action" style="width:auto;padding:4px 12px;font-size:11px;background:#333;color:#e74c3c;margin-left:4px" onclick="telegramKopar()">Baglantiyi Kes</button>' +
        '</div>'
      :
        '<div style="margin-bottom:8px">' +
          (tg.baglanti_kodu ?
            '<div style="background:#0a0a0a;border:1px solid #333;border-radius:6px;padding:10px;text-align:center">' +
              ...
              '<div style="font-size:11px;color:#555;margin-top:6px">Bot: <a href="https://t.me/palantis_game_bot" target="_blank" style="color:var(--race-color)">@palantis_game_bot</a></div>' +
            '</div>'
          :
    ...
    '<div class="card" id="ayar-sec-bildirim">' +
      ...
      '<div style="font-size:11px;color:#666;margin-bottom:8px">Hangi olaylarda Telegram bildirimi almak istiyorsunuz?</div>' +
  ...
  try {
    ...
    if (data.tatil_modu) {
      el.innerHTML = '<div style="background:rgba(46,204,113,0.1);border:1px solid rgba(46,204,113,0.3);border-radius:6px;padding:10px;margin-bottom:8px">' +
        ...
        '<div style="font-size:11px;color:#666;margin-top:4px">(Iptal sonrasi ' + (data.config?.tatil_cooldown||48) + ' PG cooldown baslar)</div>' +
      '</div>';
    } else {
      ...
      el.innerHTML = cooldownMsg +
        ...
        '<div style="display:flex;align-items:center;gap:6px;margin-bottom:6px">' +
          ...
          '<span style="font-size:11px;color:#555">(' + (data.config?.tatil_min_sure||24) + '-' + (data.config?.tatil_max_sure||336) + ')</span>' +
        '</div>' +
  ...
  try {
    ...
    if (data.aktif_token) {
      ...
      if (data.aktif_token.izinler) {
        ...
        izinGoster = '<div style="font-size:11px;color:#888;margin-top:4px;display:flex;flex-wrap:wrap;gap:3px">' +
      ...
      el.innerHTML = '<div style="background:rgba(212,162,87,0.1);border:1px solid rgba(212,162,87,0.3);border-radius:6px;padding:10px">' +
        ...
        '<div style="font-size:11px;color:#888">Bitis: ' + new Date(data.aktif_token.bitis).toLocaleString('tr-TR') + '</div>' +
        izinGoster +
        '<button class="btn-action" style="margin-top:6px;width:auto;padding:4px 10px;font-size:11px;background:#c0392b" onclick="bakicilikIptal()">Tokeni Iptal Et</button>' +
      '</div>';
    } else {
      var izinListesi = [
      ...
      el.innerHTML =
        ...
        '<div style="display:flex;align-items:center;gap:6px;margin-bottom:6px">' +
          ...
          '<button onclick="bakiciAra()" style="padding:3px 8px;background:#333;border:1px solid #555;color:#ddd;border-radius:3px;cursor:pointer;font-size:11px">Ara</button>' +
        '</div>' +
        '<div id="bakici-sonuc" style="font-size:11px;margin-bottom:6px;min-height:14px"></div>' +
        ...
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:2px;margin-bottom:8px">' +
          izinListesi.map(function(iz) {
            ...
            return '<label style="font-size:11px;color:#888;display:flex;align-items:center;gap:4px;padding:2px 0;cursor:pointer">' +
...
async function bakiciAra() {
  ...
  try {
    ...
    sonuc.innerHTML = data.map(function(p) {
      return '<span style="cursor:pointer;padding:2px 6px;border:1px solid #333;border-radius:3px;margin:1px;display:inline-block" onclick="document.getElementById(\'bakici-hedef-id\').value=\'' + p.id + '\';document.getElementById(\'bakici-sonuc\').innerHTML=\'<span style=color:#2ecc71>Secildi: ' + p.kullanici_adi + '</span>\'">' + p.kullanici_adi + ' <span style="color:#555;font-size:11px">(C' + (p.cag||1) + ')</span></span>';
    }).join(' ');
